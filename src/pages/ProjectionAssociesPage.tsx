...
export default function ProjectionAssociesPage() {
  ...
  // Associés personnes physiques from project config
  const physicalAssociates = state.associes.associes.filter((a) => a.type === "PHYSIQUE");

  const waterfall = generateWaterfall(projectionYears);
          ...
          {physicalAssociates.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {physicalAssociates.map((inv) => {
                const ownership = inv.partExploitation + inv.partFonciere;
                return (
                  <Card key={inv.id}>
                    <CardHeader className="pb-3">
                      <CardTitle className="flex items-center justify-between text-base">
                        {inv.prenom ? `${inv.prenom} ${inv.nom}` : inv.nom}
                        <Badge variant="outline">{(ownership * 100).toFixed(0)} %</Badge>
                      </CardTitle>
                    </CardHeader>
